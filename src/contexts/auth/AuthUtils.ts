...
export default class AuthUtils {
    static prepareDataUser = (data: UserType) => {
      let dadosUsuario = data;
      if (dadosUsuario && dadosUsuario.cpf) {
        dadosUsuario.icon = `${urlsServices.SIGUWS}icon?u=${getEncodeAvatarUsuario(dadosUsuario.cpf, '_80')}`;
        if (dadosUsuario.perfis) {
          let thisPerfis: string[] = [];
          dadosUsuario.perfis.forEach((perfil, index) => {
            if (perfil.sistema.descricao.toUpperCase() === sistemaNameSSO.toUpperCase()) {
              if (!thisPerfis.includes(perfil.descricao.toUpperCase())) thisPerfis.push(perfil.descricao.toUpperCase());
            }
          });
