...
import { getParameterUrl, removeParameterUrl } from "@/utils/UtilsSistema";
...
export const AuthProvider = ({ children }: { children: JSX.Element }) => {
  ...
  const validaToken = async (token: string) => {
    await apiAxios.validaTokenSSO(token).then((res) => {
      ...
      message.error({ content: 'Erro ao tentar validar seu token! Você será redirecionado.', duration: 5 }).then(() => {
  ...
  if (localStorage.getItem('token_sso') && localStorage.getItem('token_sso') != '' && localStorage.getItem('token_sso') != null && localStorage.getItem('token_sso') != undefined) {
