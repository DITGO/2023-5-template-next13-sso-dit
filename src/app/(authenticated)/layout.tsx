...
export default function RootLayout({
  ...
  children: JSX.Element
    ...
    const [unidade, setUnidade] = useState<{nome: string, codigo: number} | null>(null);
    const [itemsMenu, setItemsMenu] = useState<MenuItem[]>([]);
    const [migalhas, setMigalhas] = useState<[{title: string}]>([{title: ''}]);
                ...
                auth?.validado ? (
                    <ConfigProvider locale={ptBR}>
                        <Layout style={{ height: '100vh', width: '100vw' }}>
                            ...
                            >
                                <div className="logo" />
                                <Row style={{ fontWeight: 700, display: 'flex', justifyContent: 'center' }}>PCGO</Row>
                                <Menu
                                ...
                                <Footer>
                                    <Row style={{ width: '100%', display: 'flex', justifyContent: 'center', flexDirection: 'column', marginTop: -15 }}>
                                        <Tooltip title="Divisão de Inovação e Tecnologia da PCGO">
                                            <Row style={{ fontSize: 18, fontWeight: 700, display: 'flex', justifyContent: 'center' }}>DIT</Row>
                                        </Tooltip>
                                        <Row style={{ fontSize: 12, fontWeight: 300, display: 'flex', justifyContent: 'center' }}>{sistemaVersao}</Row>
                                    </Row>
                                </Footer>
                            </Sider>
                                ...
                                <Header style={{ padding: 5, background: colorBgContainer, borderRadius: '0px 0px 10px 10px', marginInline: 5}} >
                                    <Row style={{ width: '100%', display: 'flex', justifyContent: 'left', alignItems: 'center', padding: 5 }}>
                                        ...
                                        <Col span={10}>
                                            ...
                                            <Row>
                                                <Title style={{ fontSize: 18 }}><Button style={{ color: '#000000', fontWeight: 600 }} type='link' onClick={() => openModalSelectUnidade()}>{unidade?.nome}</Button></Title>
                                            </Row>
                                        </Col>
                                        <Col span={8} style={{ display: 'flex', justifyContent: 'right' }}>
                                            <Space>
                                                <Avatar src={auth?.user?.icon} style={{ marginRight: 5, marginTop: '-10px' }}>
                                                    {auth?.user?.nome} - {auth?.user?.funcao}
                                                </Avatar>
                                                <Col>
                                                    <Row onClick={() => actionDrawer()} className='click'><strong style={{ marginRight: 5 }}>{auth?.user?.nome}</strong></Row>
                                                    {visibleSensive ? (
                                                        <Row><Col style={{ fontSize: 12, color: '#00000080' }}>{auth?.user?.cpf} - {auth?.user?.funcao}</Col><Col style={{ marginTop: -2, paddingLeft: 5 }}><EyeInvisibleOutlined onClick={() => setVisibleSensive(!visibleSensive)} className='click'/></Col></Row>
                                                    ) : (
                        ...
                        <Drawer title="Dados do Usuário" placement="right" onClose={actionDrawer} open={openDrawer}>
                            ...
                            <p><strong>Sistema / Perfil</strong></p>
                            <p>&nbsp;</p>
